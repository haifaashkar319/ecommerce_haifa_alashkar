...
def test_deduct_item_insufficient_stock(client):
    ...
    deduct_response = client.post(f'/inventory/{item_id}/deduct', json={"quantity": 15})  # Exceeds stock
    assert deduct_response.status_code == 400  # Expect a 400 Bad Request
    assert deduct_response.json["error"] == "Quantity exceeds stock availability"
